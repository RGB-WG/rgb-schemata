...
pub fn udc_schema() -> SubSchema {
    let types = StandardTypes::with(rgb21_stl());
    
    let code = rgbasm! {
        // SUBROUTINE 1: genesis validation
        // Num. tokens in global state into a8[2]
        cng     0x0836,a8[2]                    ;
        // Num. tokens in contract state into a16[0]
        cnc     0x0FA0,a16[0]                   ;
        cnv     a8[2],a16[2]                    ;
        eq.n    a16[2],a16[0]                   ;
        ...
        // message
        put     s16[0],"token index and allocations need parity";
        fail                                    ;         
        // offset_0x18 (pos 24):
        // Sub 0x1 to a[1] for access global states as array
        sub     a8[2],0x1                       ;
        // Put 0x0 to a[0] for iterate global states
        put     a8[0],0x0                       ;
        // offset 0x001F (pos 31):
        // Put 0x0 to a[0] for iterate contract states
        put     a8[1],0x0                       ;
        // offset 0x0023 (pos 35)                          
        // check that token global state is less then contract state
        gt.u    a8[1],a8[2]                     ;
        // if they do, jump to the next check
        jif     0x0057                          ;
        // Read global state with index a[0] into s[1]
        ldg     0x0836,a8[0],s16[2]             ; 
        // Put 0 to a16[0] for offset global state data
        put     a16[0],0x00                     ;
        // Extract 128 bits from the beginning of s[1] into r128[1]
        extr    s16[2],r128[1],a16[0]           ;
        // a32[0] now has token index from global state
        spy     a32[0],r128[1]                  ;
        // Read owned state with index a[0] into s[0]
        cnv     a8[1],a16[1]                    ;
        // Read contract state with index a[1] into s[2]
        lds     0x0FA0,a16[1],s16[2]            ;
        // Put 0 to a16[0] for offset global state data
        put     a16[0],0x00                     ;
        // Extract 128 bits from the beginning of s[1] into r128[1]              
        extr    s16[2],r128[1],a16[0]           ;
        ...
        // if they do, jump to the next check
        jif     0x005E                          ;
        // if not, inc +1 in contract state count
        add     a8[1],0x1                       ;
        jmp     0x0023                          ;
        // offset_0x0057 (pos 87):
        // we need to put a string into first string register which will be used as an error
        // message
        put     s16[0],"the allocated token has an invalid index";
        fail                                    ;
        // offset_0x005E (pos 94):
        // Put 4 to a16[0]
        put     a16[0],4                        ;
        ...
        eq.n    a64[0],a64[1]                   ;
        // if yes, jump to next verification    
        jif     0x0079                         ;
        // terminate the subroutine
        put     s16[0],"owned fraction is not 1";
        fail                                    ;
        // offset_0x0079 (pos 121):
        // do we check all tokens? 
        eq.n    a8[2],a8[0]                     ;
        // if yes, jump to next finish program
        jif     0x0085                          ;
        // if no, iterate next token index 
        add     a8[0],0x1                       ;      
        jmp     0x001F                          ;        
        // offset_0x0085 (pos 133):
        // terminate the subroutine
        ret                                     ;
    ...
    Schema {
        ...
        global_types: tiny_bmap! {
            ...
            GS_TOKENS => GlobalStateSchema::many(types.get("RGB21.TokenData")),
...
pub fn udc_rgb21() -> IfaceImpl {
    let schema = udc_schema();
    let iface = rgb21();
